...
// максимальное число командиров и рядовых в одной армии
constexpr int MaxCom = 8;
constexpr int MaxBar = 8;
...
void B_LocalSendToLink(Brigade* BR)
{
	...
	// 4) обновление PrevSumm*
	BLS->PrevSumm3 = BLS->PrevSumm2;
	BLS->PrevSumm2 = BLS->PrevSumm1;
	BLS->PrevSumm1 = static_cast<int>(S);
}
...
void B_WideLocalSendToLink(Brigade* BR)
{
	...
	if (BLS->time >= 0 && tmtmt - BLS->time > 350)
	{
		zastr = true;
		BLS->time = -10;
	}
		...
		if (PosChanged)
		{
			for (int i = 0; i < N && i < Nmx; ++i)
			{
				...
				if (OB && OB->Serial == BR->MembSN[i])
					OB->NewMonsterSendTo(BLS->Position[ti] << 4,
						BLS->Position[ti + 1] << 4,
						prio, 0);
			}
		}
...
// ——— Безопасная версия ApplySwap ———
void ApplySwap(Brigade* BR, word* swp)
{
	if (!BR || !BR->BOrder)  // защита от нулевого указателя
		return;
	...
	// Динамические буферы вместо word tm[512];
	word* tm = static_cast<word*>(malloc(N * sizeof(word)));
	word* tmsn = static_cast<word*>(malloc(N * sizeof(word)));
	word* tm1 = static_cast<word*>(malloc(N * sizeof(word)));
	word* tmsn1 = static_cast<word*>(malloc(N * sizeof(word)));
	if (!tm || !tmsn || !tm1 || !tmsn1)
	{
		free(tm); free(tmsn); free(tm1); free(tmsn1);
		return;
	}

	OrderDescription* ODE = ElementaryOrders + BR->WarType - 1;
	// Увеличиваем NMemb, если нужно
	if (N - 2 < ODE->NUnits)
	{
		int add = (ODE->NUnits + 2 - N);
		memset(BR->Memb + N, 0xFF, add * sizeof(word));
		memset(BR->MembSN + N, 0xFF, add * sizeof(word));
		BR->NMemb = ODE->NUnits + 2;
		N = BR->NMemb;
	}

	// Запоминаем старые и новые индексы
	for (int i = 2; i < N; i++)
	{
		int idx = swp[i - 2] + 2;
		if (idx < N)
...
// ——— Обновлённая HumanEscapeLink с защитой от нулевых указателей ———
void HumanEscapeLink(Brigade* BR)
{
	if (!BR || !BR->BOrder)
		return;

	B_HSend* BS = static_cast<B_HSend*>(BR->BOrder);
	if (BR->NMemb < 4)
	{
		BR->DeleteBOrder();
		return;
	...
	byte Dir = (BS->Dir > 256)
		? GetDir((x - xc) >> 4, (y - yc) >> 4)
		: static_cast<byte>(BS->Dir);

	char dd = char(Dir) - char(BR->Direction);
	byte pri = BS->Prio;

	// Логика разворота и свопа аналогична ApplySwap
	if (abs(dd) < 32)
	{
		BR->CreateSimpleOrderedPositions(x, y, Dir);
		BR->Direction = Dir;
	...
	else
	{
		...
		{
			if (dd > 64 || dd < -64)
				ApplySwap(BR, ODE->SymInv);
		}
		// после свопа или простой смены направления
		BR->CreateSimpleOrderedPositions(x, y, Dir);
		BR->Direction = Dir;
	}

	// Доставляем отряд на новые позиции
	for (int i = 0; i < BR->NMemb; i++)
	{
		word MID = BR->Memb[i];
		if (MID != 0xFFFF)
		{
			OneObject* OB = Group[MID];
			if (OB && !OB->Sdoxlo)
				OB->NewMonsterPreciseSendTo(int(BR->posX[i]) << 4,
					int(BR->posY[i]) << 4,
					pri, 0);
		}
	}
...
{
	// 1) Защита от нулевого указателя
	if (!OBJ || !OBJ->LocalOrder)
		return;

	Order1* ord = OBJ->LocalOrder;
	word ArmID = ord->info.BuildObj.ObjIndex;
	bool OFC = (OBJ->Ref.General && OBJ->Ref.General->OFCR != nullptr);

	// 2) Если ещё не в армии — ищем, куда присоединиться
	if (ArmID == 0xFFFF)
	{
		int Aid = -1;
		...
		if (Aid != -1)
		{
			...
			ord->info.BuildObj.ObjIndex = Aid;
		}
		return;
	}
...
void SearchArmy(OneObject* OB)
{
	if (!OB) return; // дополнительная проверка на валидность объекта

	Order1* OR1 = OB->CreateOrder(0);
	if (!OR1) return; // проверка на nullptr
